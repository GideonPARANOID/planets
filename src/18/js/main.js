   ...
   items = (function generateItems(json) {
      ...
      for (var i = 0; i < json.length; i++) {
         ...
         result[i] = current.type === 'Star' ? 
            new Star(current.radius, genPathTA(current.colorMapURL), current.daysPerYear, 
               generateItems(current.satellites)) :
            new Planet(current.radius, genPathTA(current.colorMapURL), 
               genPathT(current.specularMapURL), current.shininess, genPathT(current.darkMapURL),
               current.distance, current.eccentricity, current.orbitAngle, current.orbitOffset, 
               current.velocity, current.daysPerYear, current.axis, (current.rings ? 
                  new Rings(current.rings.size, genPathTA(current.rings.colorMapURL)) : 
                  null), generateItems(current.satellites));
      }
...
function initialiseBackground() {
   ...
   background = new Cube(BOUNDS * 2, genPathTA(['outerspace.jpg']), null, 0, null, null);
}
...
function initialiseControls(canvas) {
   ...
   document.getElementById('cubiverse').addEventListener('change', toggleCubiverse, false);


   function toggleCubiverse(event) {
      CUBIVERSE = event.target.checked;
...
function genPathTA(fileName) {
   var res = [];

   if (fileName && fileName.length) {
      for (var i = 0 ; i < fileName.length; i++) {
         res[i] = TEXTUREPATH + fileName[i];
      }
   }
